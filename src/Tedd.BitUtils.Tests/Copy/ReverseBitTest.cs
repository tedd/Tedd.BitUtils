...
namespace Tedd.BitUtils.Tests.Copy
{
    public class ReverseBitTest
    {
        ...
        public void TestInt16(Int16 r)
        {
            ...
            var n = r.ReverseBitsCopy();
            Assert.Equal(o, r);
            var actual = new string(Convert.ToString((uint)n & 0xFFFF, 2).PadLeft(sizeof(Int16) * 8, '0').ToArray());
            Assert.Equal(expected, actual);
        }
        ...
        public void TestUInt32(UInt32 r)
        {
            ...
            var n = r.ReverseBitsCopy();
            Assert.Equal(o, r);
            var actual = new string(Convert.ToString((long)n, 2).PadLeft(sizeof(UInt32) * 8, '0').ToArray());
            Assert.Equal(expected, actual);
        }
