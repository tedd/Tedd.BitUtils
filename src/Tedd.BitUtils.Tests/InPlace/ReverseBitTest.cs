    ...
    public class ReverseBitTest
    {
        ...
        public void TestInt16()
        {
            for (var i = 0; i < 100; i++)
            {
                ...
                r.ReverseBits();
                var actual = new string(Convert.ToString(r, 2).PadLeft(sizeof(Int16) * 8, '0').ToArray());
                Assert.Equal(expected, actual);
            }
        }
        [Fact]
        ...
        public void TestUInt32()
        {
            for (var i = 0; i < 100; i++)
            {
                ...
                r.ReverseBits();
                var actual = new string(Convert.ToString(r, 2).PadLeft(sizeof(UInt32) * 8, '0').ToArray());
                Assert.Equal(expected, actual);
            }
        }
        ...
        public void TestUInt64()
        {
            for (var i = 0; i < 100; i++)
            {
                ...
                r.ReverseBits();
                var actual = new string(Convert.ToString(r, 2).PadLeft(sizeof(UInt64) * 8, '0').ToArray());
                Assert.Equal(expected, actual);
            }
        }
    }
